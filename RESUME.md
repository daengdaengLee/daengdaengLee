...
<!--
@TODO
연구 및 개발 전체 과정에서 주도적인 역할 강조!!
PDF 스펙 문서를 보고 연구 및 실험을 통해 원하는 기능 개발 
이미지 데이터 중복 안 하고 래퍼런스로 출력하는 기능에 대한 이야기 
Node.js 스트림을 사용해서 큰 용량의 PDF 파일도 생성할 수 있었다는 이야기
헤드리스 브라우저 사용한 이야기
테스트 부분 -> outbound 영역 분리해서 로컬에서 파일 생성해볼 수 있게 한 부분 
-->
...
<!--
@TODO
연구 및 개발 전체 과정에서 주도적인 역할
SVG 스펙 및 웹 API 에 대한 연구 및 실험을 통해 원하는 기능 개발 
-->
...
- 파이썬 스크립트와 실행 옵션을 노드로 등록하고 노드를 연결하여 워크플로우를 구성하는 UI를 개발했습니다. (React, SVG)
- 웹 소켓으로 워크플로우 실행 로그를 받아 출력하는 콘솔 화면 UI를 개발했습니다. (React, WebSocket)
- 여러 UI의 상태와 이벤트를 서버와 동기화하기 위해 전역 상태 관리 도구로 Redux와 Redux-Saga를 도입했습니다.
- IIFE 모듈 패턴과 JQuery를 사용하여 2개 파일(각 5만, 6만줄)로 구성된 프로젝트를 Webpack, Babel, React 를 사용도록 변경했습니다.
  그 결과 목적과 역할에 맞게 모듈 및 파일을 분리할 수 있게 되었고 서로 협업하는 과정에서 불필요한 충돌이 줄어들었습니다.
...
- 해시 코드는 URL 에서 안전하게 사용 가능하도록 Base62 방식으로 인코딩한 문자를 사용합니다.
- 해시 충돌이 발생하면 원본 URL에 salt값을 더해 다시 단축 알고리즘을 실행합니다.
  사용한 단축 URL이 아직 적기 때문에 이 방법으로 충분히 해시 충돌을 해소할 수 있다고 생각합니다.
